...
def generate_seasonal_data(seed=42):
    """Full-year synthetic dataset with realistic Sri Lanka seasonal cycle.

    Climatological pattern:
      Jan–Feb : mostly cool (Cat 1–2)
      Mar–May : inter-monsoonal heat peak (Cat 3–4 elevated)
      Jun–Sep : SW Monsoon (moderate, Cat 2–3)
      Oct–Nov : NE Monsoon onset (Cat 1–2)
      Dec     : cool dry season
    ...
    # Seasonal heat index signal (sinusoidal with inter-monsoonal peak ~doy 100)
    hi_signal = (
        38.0                                         # baseline
        + 8.0 * np.sin(2 * np.pi * (doy - 60) / 365)  # annual cycle peaking ~May
        + 4.0 * np.exp(-0.5 * ((doy - 100) / 20)**2)  # Mar-May sharp peak
        - 3.0 * np.exp(-0.5 * ((doy - 200) / 40)**2)  # SW Monsoon cooling
        + rng.normal(0, 2, n)                           # daily noise
    )
...
def chart_seasonal_calendar(test_data, issued_warnings, hi_signal=None, dates=None, suffix=''):
    ...
    fig.suptitle('Sri Lanka Heat IBF — Anuradhapura Pilot\nWarning Calendar & Seasonal Pattern',
...
def chart_advisory_panel(train_data, forecast_p, scenario_label='Scenario A', suffix=''):
    ...
    ax0.text(0.5, 0.20, f'Anuradhapura District — 48-hour advisory',
...
def write_csv_template():
    ...
    path = os.path.join(OUT_DIR, 'anuradhapura_data_template.csv')
...
def main():
    ...
    print('  Anuradhapura District Pilot')
    print('='*60)
